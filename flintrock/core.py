...
def ensure_java8(client: paramiko.client.SSHClient):
    ...
    # print("java_installer ={}, python={}, install={}".format(user_config.java_installer, user_config.python_cmd, user_config.install_cmd), file=sys.stderr)
    if not java_major_version or java_major_version < (1, 8):
        logger.info("[{h}] Installing Java 1.8...".format(h=host))

        ssh_check_output(
            client=client,
            command= user_config.java_installer
            )
...
def setup_node(
    ...
    storage_dirs_raw = ssh_check_output(
        ...
        """.format(python=user_config.python_cmd))
