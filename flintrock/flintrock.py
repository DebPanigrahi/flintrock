...
def set_user_config(*, 
    ...
    if java_installer: 
        user_config.java_installer = java_installer
    if python_cmd: 
       user_config.python_cmd = python_cmd
    if install_cmd:
       user_config.install_cmd = install_cmd
    if ports:
       user_config.ports = [x.split(':') for x in ports.replace(' ', '').split(',')] 
    user_config.client_ip = client_ip
    user_config.aws_credentials = aws_credentials
...
@click.option('--ec2-tag', 'ec2_tags',
              ...
              help="Additional tags (e.g. 'Key,Value') to assign to the instances. "
                   "You can specify this option multiple times.")
@click.option('--java-installer', 'java_installer', 
              help="commands to auto install java without user input.")
@click.option('--python-cmd', 'python_cmd', default='python', 
              help="python shell command python(RHL) or python3(UBUNTU).")
@click.option('--install-cmd', 'install_cmd', default='yum',
              help="linux app install command yum (RHL) or apt-get install --assume-yes (UBUNTU).")
@click.option('--ports', 'ports', 
              help="ports to open, format: port1-type:port1#:port1#, port2-type:port2#:port2#")
@click.option('--client-ip', 'client_ip', 
              help="client ip like 0.0.0.0/0")
@click.option('--aws-credentials', 'aws_credentials', 
              help="aws credentials environment setup")
@click.pass_context
def launch(
        cli_context,
        cluster_name,
    ...
    set_user_config(java_installer=java_installer,
    ...
    if provider == 'ec2':
        cluster = ec2.launch(
            ...
            tags=ec2_tags,
            ports=user_config.ports, 
            client_ip=user_config.client_ip)
    else:
