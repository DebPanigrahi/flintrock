...
class HDFS(FlintrockService):
    ...
    def install(
        ...
        ssh_check_output(
            ...
            """.format(python=user_config.python_cmd, version=self.version, download_source=self.download_source))
        ...
        try:
            if self.version:
                ...
                ssh_check_output(
                    ...
                    command="""
                        set -e
                        sudo {install_app} install -y git
                        sudo {install_app} install -y java-devel
                        """.format(install_app=user_config.install_cmd))
                ssh_check_output(
                    client=ssh_client,
                    command="""
                        set -e
            ...
            ssh_check_output(
                ...
                command="""
                    ...
                    {aws_credentials}
                """.format(aws_credentials=user_config.aws_credentials))
        except Exception as e:
