...
def get_or_create_flintrock_security_groups(
        *,
        ...
        region, 
        ports,
        client_ip) -> "List[boto3.resource('ec2').SecurityGroup]":
    """
    ...
    if client_ip: 
        flintrock_client_cidr = client_ip
    else:
        flintrock_client_ip = (
    ...
    for port in ports:
        client_rules += [SecurityGroupRule(
            ip_protocol=port[0],
            from_port=int(port[1]),
            to_port=int(port[2]),
            cidr_ip=flintrock_client_cidr,
            src_group=None)]
...
def _create_instances(
        *,
        ...
        user_data, 
        min_root_ebs_size_gb) -> 'List[boto3.resources.factory.ec2.Instance]':
    ec2 = boto3.resource(service_name='ec2', region_name=region)

    cluster_instances = []
    spot_requests = []

    block_device_mappings_master = get_ec2_block_device_mappings(
        min_root_ebs_size_gb=min_root_ebs_size_gb,
        ami=ami_master,
        region=region)
    ...
    try:
        if spot_price:
            ...
            cluster_instances = ec2.create_instances(
                ...
                KeyName=key_name,
                InstanceType=instance_type,
                BlockDeviceMappings=block_device_mappings_master,
                Placement={
                    'AvailabilityZone': availability_zone,
                    'Tenancy': tenancy,
                    'GroupName': placement_group},
                ...
                UserData=user_data)           #       left stranded.

            cluster_instances += ec2.create_instances(
                MinCount=num_instances-1,
                MaxCount=num_instances-1,
                ImageId=ami,
                KeyName=key_name,
                InstanceType=instance_type,
                BlockDeviceMappings=block_device_mappings,
